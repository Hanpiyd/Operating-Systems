...
alloc_proc(void)
{
    ...
    if (proc != NULL)
    {
        // LAB4:EXERCISE1 YOUR CODE 2210211
        /*
...
copy_thread(struct proc_struct *proc, uintptr_t esp, struct trapframe *tf)
{
    ...
    // 将新进程的trapframe指针设置为其内核栈顶的位置。
    // 减去sizeof(struct trapframe)是为了确保在内核栈的顶部分配一块空间来存储陷阱帧。
    // 陷阱帧用来保存进程在进入内核模式时的上下文信息
    *(proc->tf) = *tf;
    // 这行代码将当前进程的trapframe（即父进程的上下文）复制到新进程的trapframe中。
    // 子进程从父进程继承了相同的寄存器状态，确保当新进程执行时，能够继续执行与父进程相似的操作。
...
int do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf)
{
    ...
    ret = -E_NO_MEM;
    // LAB4:EXERCISE2 YOUR CODE 2210211
    /*
     * Some Useful MACROs, Functions and DEFINEs, you can use them in below implementation.
     * MACROs or Functions:
